...
Las redes DWDM y los amplificadores ópticos utilizan láseres infrarrojos (en la Banda C) que son **invisibles al ojo humano** y pueden causar daño ocular irreversible[cite: 1].
* **Regla de oro:** Nunca mirar directamente al extremo de una fibra óptica, un conector o un puerto activo del equipo.
...
La suciedad (polvo, grasa) es la principal causa de fallas en redes ópticas, ya que genera atenuación y reflexiones que degradan el OSNR[cite: 1].
* Inspeccionar siempre el núcleo del conector con un microscopio óptico antes de conectarlo.
* Limpiar la férula utilizando lápices limpiadores mecánicos (tipo *One-Click Cleaner*) o toallitas libres de pelusa con alcohol isopropílico de alta pureza.
...
Es crítico identificar y no mezclar los tipos de pulido de las férulas, ya que una conexión cruzada destruirá ambos conectores[cite: 1]:
...
El proceso de aprovisionamiento en el chasis HT6000 permite establecer el puente entre la red del cliente y la red de transporte DWDM[cite: 1].
...
3. **Configuración de Transceptores:** Activar los puertos de cliente (ej. SFP+ 10G) y configurar la tasa de bits y el protocolo esperado (ej. 10GbE LAN).

### Mapeo de Puertos y Asignación de Canales
1. **Mapeo Cliente a Línea (Cross-connection):** Configurar lógicamente el transpondedor para que el tráfico ingresado por el puerto cliente "Tx/Rx 1" sea encapsulado y mapeado hacia la interfaz de línea DWDM "Line 1"[cite: 1].
2. **Asignación del Canal (Longitud de Onda):** En las interfaces de línea sintonizables (Tunable XFP/SFP+), seleccionar la longitud de onda específica según la grilla ITU-T G.694.1 (ej. Canal 21, 192.1 THz, 1560.61 nm)[cite: 1].
...
2. Insertar el **OVA JW3303** en serie con el enlace, respetando los puertos de entrada (IN) y salida (OUT)[cite: 1].
3. Encender el OVA y configurarlo inicialmente con una atenuación de **0 dB**.
4. Incrementar gradualmente la atenuación (en pasos de 1 dB) para simular atenuación en la línea y evaluar el margen de operación del sistema[cite: 1].

### 3.2 Obtención del Espectro Óptico (OSA RXT4510)
1. Conectar el puerto de entrada del **RXT4510** al puerto de monitoreo (*MON*) del chasis HT6000 o mediante un *splitter* óptico en la línea[cite: 1].
...
4. Medir y registrar la **Potencia por Canal (dBm)** y la **Relación Señal a Ruido Óptica (OSNR)** de cada portadora para certificar la viabilidad del enlace[cite: 1].

### 3.3 Medición de Rendimiento y BER (MTX150x)
1. Conectar el puerto de prueba del **MTX150x** al puerto de cliente del Switch CSS610 (o directamente al transpondedor en configuración back-to-back)[cite: 1].
2. Configurar la prueba **RFC 2544**: Definir el tamaño de las tramas (ej. 64, 512, 1518 bytes) y el tiempo de prueba.
3. Ejecutar el test de rendimiento para verificar la tasa máxima de transferencia (*Throughput*) y registrar la **Pérdida de Tramas (Frame Loss)**[cite: 1].
4. Configurar y ejecutar una prueba de **Bit Error Rate (BER)** por un periodo prolongado (ej. 15-30 minutos) para certificar que el transporte de la capa física esté libre de errores (BER ideal = 0)[cite: 1].
